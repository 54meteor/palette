...
class Coverswf(Contracts):
    ...
    def deploy_contract_no_owner(self,_swf, _rswf, _convertRate):
        ...
        tx_hash = self.w3.eth.contract(
            abi=self.abi,
            bytecode=self.bin).constructor(_swf, _rswf, _convertRate).transact()
    ...
    def testCheckExchange(self,amount):
        c =  self.function.checkExchange(amount).call()
        self.printO("can exchange: " + str(c / self.DECIMAL))
        return c


    def testGetAvailable(self):
        avai =  self.function.getAvailable().call()
        self.printO("available: " + str(avai / self.DECIMAL))
        return avai

    def testExchange(self,amount):
        self.printO("exchange start")
        self.function.exchange(amount).transact()
        self.printO("exchange end")
