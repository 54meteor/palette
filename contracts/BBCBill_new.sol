...
contract BBCBill is Ownable {
    ...
    event retraceEvent(uint256 _txid, uint256 _amount, address payable _walletAddress, address _contractAddress,
        uint256 _fee);
    event withdrawEvent(string _txid, string _bbcBranch, address payable _walletAddress,
        address _contractAddress, uint256 _amount, uint256 _fee, uint256 _rate);
    ...
    function _withDraw() internal onlyAdmin{
        if(_contractAddress == address(0)){
            msg.sender.transfer(_amount);
        }else{
            if(_compatible[_contractAddress]){
