...
contract CocoStakingMaster {
    ...
    struct PoolInfo {
        ...
        uint256 accPerShare; // Accumulated cocos per share, times 1e12.
    }
    ...
    function setCocoPerBlock(uint256 _newPerBlock) public onlyManager {
        massUpdatePools();
        cocoPerBlock = _newPerBlock;
    }
    ...
    // Update the given pool's coco allocation point. Can only be called by the owner.
    ...
    function reward(uint256 blockNumber) public view returns (uint256) {
        ...
        return cocoPerBlock.div(2**_phase);
    }

    function getCocoBlockReward(uint256 _lastRewardBlock)
        public
        view
        returns (uint256)
    {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 blockReward = getCocoBlockReward(pool.lastRewardBlock);
        if (blockReward <= 0) {
            return;
        }
        uint256 cocoReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
        bool minRet = ICocoToken(nameAddr("COCOTOKEN")).mint(address(this), cocoReward);
        //bool minRet;
        if (minRet) {
            pool.accPerShare = pool.accPerShare.add(cocoReward.mul(1e12).div(lpSupply));
        }
        pool.lastRewardBlock = block.number;
    }

    // View function to see pending  on frontend.
    function pendingCoco(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (user.amount > 0) {
            if (block.number > pool.lastRewardBlock) {
                uint256 blockReward = getCocoBlockReward(pool.lastRewardBlock);
                uint256 cocoReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
                accPerShare = accPerShare.add(cocoReward.mul(1e12).div(lpSupply));
    ...
    // Deposit LP tokens to Master for coco allocation.
    function deposit(uint256 _pid, uint256 _amount) public notPause {
        ...
        if (user.amount > 0) {
            ...
            if (pendingAmount > 0) {
                safeCocoTransfer(msg.sender, pendingAmount);
            }
        }
        if (_amount > 0) {
    ...
    function safeCocoTransfer(address _to, uint256 _amount) internal {
        uint256 joyBal = ICocoToken(nameAddr("COCOTOKEN")).balanceOf(address(this));
        if (_amount > joyBal) {
            ICocoToken(nameAddr("COCOTOKEN")).transfer(_to, joyBal);
        } else {
            ICocoToken(nameAddr("COCOTOKEN")).transfer(_to, _amount);
        }
    }
